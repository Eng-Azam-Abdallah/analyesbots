...
### يعمل فعليًا
- Backend مع مزامنة Rexovaan و Zoom Store و TechnySoft و Telegram Buyer و ShopDigital و TeleShopBot.
- Frontend يعرض بيانات حية من PostgreSQL عبر NestJS.
- تسجيل التغيّرات وتتبّع الأسعار والكميات.
- صفحة بوتات تعرض المصادر وآخر مزامنة.
...
- HyleHub / HyperVin: المصدر موجود، لكن `hypervin.xyz` لا يُحلّ عبر DNS (NXDOMAIN) من بيئة التطوير الحالية.
...
1. حل وصول الشبكة إلى `canboso.com` والتحقق من أول مزامنة ناجحة.
2. تحسين عرض المقارنة بين مصادر متعددة لنفس نوع المنتج (إن لزم).
3. تنبيهات اختيارية عند طلع/نزل.
4. Auth بسيط للوحة إن أصبحت عامة.
5. دعم مصادر/بوتات إضافية بنفس نمط الـ Orchestrator.
...
| 2026-08-05 | مصادر حالية: Rexovaan Reseller + Canboso Buyer + Zoom Store + TechnySoft (@mkeshopbot) |
| 2026-08-05 | Zoom Store: مزامنة منتجات/رصيد كل دقيقة؛ بدون `purchase` |
| 2026-08-05 | TechnySoft: سعر مرجعي `price_usd`؛ بدون `POST /v1/buy` |
...
> Analyes تجمع منتجات مصادر البيع (Rexovaan و Zoom Store و @mkeshopbot و Buyer API يعملون، وCanboso جاهز بالربط وينتظر توفر الشبكة)، تحفظ التفاصيل والأسعار والكميات في PostgreSQL، تسجّل كل تغيّر تلقائيًا كل دقيقة، وتعرض النتيجة في واجهة عربية حية.
