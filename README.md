...
- `QAMIFY_API_URL` / `QAMIFY_API_KEY` (Qamify)
- `INSIGHTX_API_URL` / `INSIGHTX_API_KEY` (InsightX Store)
- `AIMARKET_API_URL` / `AIMARKET_API_KEY` (Ai-Market / @Maher92Bot)
- `VEXORAN_API_URL` / `VEXORAN_API_KEY` (Vexoran Shoppie / @VexoranShoppieBot)
- `EMSTORE_API_URL` / `EMSTORE_API_KEY` (EM Store helper / @EliteMethodsStoreBot)
- أسماء البوتات: `RESELLER_BOT_USERNAME` / `CANBOSO_BOT_USERNAME` / `ZOOMSTOORE_BOT_USERNAME` / `TECHNYSOFT_BOT_USERNAME` / `TELEGRAM_BUYER_BOT_USERNAME` / `HYPERVIN_BOT_USERNAME` / `SHOPDIGITAL_BOT_USERNAME` / `TELESHOPBOT_BOT_USERNAME` / `QAMIFY_BOT_USERNAME` / `INSIGHTX_BOT_USERNAME` / `AIMARKET_BOT_USERNAME` / `VEXORAN_BOT_USERNAME` / `EMSTORE_BOT_USERNAME`
