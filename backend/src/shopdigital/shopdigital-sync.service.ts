...
@Injectable()
export class ShopDigitalSyncService {
  private readonly logger = new Logger(ShopDigitalSyncService.name);
  private running = false;

  constructor(
    private readonly prisma: PrismaService,
    private readonly api: ShopDigitalApiClient,
  ...
  async ensureBot() {
    ...
    return this.prisma.bot.upsert({
      ...
      create: {
        username,
        name: 'KOKORO SHOP',
        displayName: 'KOKORO SHOP',
        sourceType: 'shopdigital_api',
      },
      update: {
        sourceType: 'shopdigital_api',
        name: 'KOKORO SHOP',
  ...
  async runSync() {
    if (this.running) {
      this.logger.warn('ShopDigital sync already running — skipped');
    ...
    try {
      const [productsResponse, balanceResponse] = await Promise.all([
        ...
        this.api.getBalance().catch((error: Error) => {
          this.logger.warn(`ShopDigital balance failed: ${error.message}`);
          return null;
        }),
      ]);
      ...
      this.logger.log(
        `ShopDigital sync ok — products=${productsSeen} changes=${changesDetected}`,
      );
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unknown ShopDigital sync error';
      ...
      this.logger.error(`ShopDigital sync failed: ${message}`);
      throw error;
    } finally {
      this.running = false;
    }
  ...
  private async upsertProduct(botId: string, remote: ShopDigitalProductDto) {
    const externalKey = String(remote.id).trim();
    ...
    const description =
      this.stripHtml(remote.description_es) ||
      this.stripHtml(remote.description_en);
