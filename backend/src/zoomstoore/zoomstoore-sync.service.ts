...
@Injectable()
export class ZoomStooreSyncService {
  private readonly logger = new Logger(ZoomStooreSyncService.name);
  private running = false;

  constructor(
    private readonly prisma: PrismaService,
    private readonly api: ZoomStooreApiClient,
  ...
  async ensureBot() {
    ...
    return this.prisma.bot.upsert({
      ...
      create: {
        username,
        name: 'Zoom Store',
        displayName: 'Zoom Store',
        sourceType: 'zoomstoore_api',
      },
      update: {
        sourceType: 'zoomstoore_api',
        displayName: 'Zoom Store',
  ...
  async runSync() {
    if (this.running) {
      this.logger.warn('ZoomStoore sync already running — skipped');
    ...
    try {
      const [productsResponse, balanceResponse] = await Promise.all([
        ...
        this.api.getBalance().catch((error: Error) => {
          this.logger.warn(`ZoomStoore balance failed: ${error.message}`);
          return null;
        }),
      ]);
      ...
      this.logger.log(
        `ZoomStoore sync ok — products=${productsSeen} changes=${changesDetected}`,
      );

      return { skipped: false as const, productsSeen, changesDetected };
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'Unknown ZoomStoore sync error';
      ...
      this.logger.error(`ZoomStoore sync failed: ${message}`);
      throw error;
    } finally {
      this.running = false;
    }
  ...
  private async upsertProduct(botId: string, remote: ZoomStooreProductDto) {
    const externalKey = String(remote.id);
    const price = Number(remote.price ?? 0);
    const listPrice =
      remote.list_price === null || remote.list_price === undefined
        ? null
        : Number(remote.list_price);
    const stock = Math.max(0, Math.floor(Number(remote.stock ?? 0)));
    const currency = remote.currency || 'USDT';
    const isActive = remote.in_stock !== false;
    ...
    if (!existing) {
      const created = await this.prisma.product.create({
        data: {
          ...
          regularPrice:
            listPrice === null ? null : new Prisma.Decimal(listPrice),
          stock,
          isActive,
          rawPayload: payload,
        },
