...
import type { AiMarketProductDto } from './aimarket.types';

@Injectable()
export class AiMarketSyncService {
  private readonly logger = new Logger(AiMarketSyncService.name);
  private running = false;

  constructor(
    private readonly prisma: PrismaService,
    private readonly api: AiMarketApiClient,
  ...
  async ensureBot() {
    ...
    return this.prisma.bot.upsert({
      ...
      create: {
        username,
        name: 'Ai-Market',
        displayName: 'Ai-Market',
        sourceType: 'aimarket_api',
      },
      update: {
        displayName: 'Ai-Market',
        name: 'Ai-Market',
  ...
  async runSync() {
    if (this.running) {
      this.logger.warn('AiMarket sync already running — skipped');
    ...
    try {
      const [productsResponse, balanceResponse] = await Promise.all([
        ...
        this.api.getBalance().catch((error: Error) => {
          this.logger.warn(`AiMarket balance failed: ${error.message}`);
          return null;
        }),
      ]);
      ...
      this.logger.log(
        `AiMarket sync ok — products=${productsSeen} changes=${changesDetected}`,
      );
      ...
      this.logger.error(`AiMarket sync failed: ${message}`);
      throw error;
    } finally {
      this.running = false;
    }
  ...
  private async upsertProduct(botId: string, remote: AiMarketProductDto) {
